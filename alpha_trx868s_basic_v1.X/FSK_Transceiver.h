 ...
 * Collaborateur    : ...
 * Processor        : PIC24
 * Tools used       : MPLAB® X IDE v4.00 and MPLAB® Code Configurator (MCC) Version: 3.36
 * Compiler         : Microchip XC16 v1.32
 * Programmateur    : PICkit 3
 *
 * 
 * Information sur le module ALPHA TRX433S :
 * ----------------------------------------
 * Alpha transceivers, this device appears to be the same or very similar: RFM12B (RF12)
 * RFM12B wireless FSK transceiver module
 *  http://www.hoperf.com/rf_fsk/fsk/21.htm
 * RFM12B,FSK transceiver - FSK modulation - HOPE Microelectronics
 *  http://www.hoperf.com/rf/module/fsk/RFM12B.htm
 * RFM12 Demo Kit User Manual
 *  http://www.hoperf.com/upload/rf/RFM12TOOLS_en.pdf
 * 
 *  http://wiki.london.hackspace.org.uk/view/Project:Alpha_transceivers
 ...
 * Il n'est pas nécessaire de modifier le cablage du module lors de l'utilisation
 * d'applications à sauts de fréquence (frequency-hopping applications), le changement
 ...
 * --> mettre la broche à VDD pour désactiver l'accès directe au registre FIFO (se comporte comme un CS)
 * 
 ...
 *
 * Lecture du registre Status lorsque la broche nIRQ passe à 0
 */
...
// TODO: > Réfléchir à l'implémentation de l'envoie et de la réception d'une trame de longueur variable
//         voir "Calculating the Checksum of an API Packet" http://www.digi.com/support/kbase/kbaseresultdetl?id=2206

#ifndef FSK_TRANSCEIVER_H
#define FSK_TRANSCEIVER_H
...
/** Détection du signal nIRQ du module RF */
...
/* Définition des codes commande à titre indicatif */
// Command code (/!\ toutes les commandes ne sont pas sur 1 octet):
//#define CFG_SET_CMD_CODE                 0b10000000  // Configuration Setting Command
//#define PWR_MGMT_CMD_CODE                0b10000010  // Power Management Command
//#define FQ_SET_CMD_CODE                  0b1010      // Frequency Setting Command
//#define DATA_RATE_CMD_CODE               0b11000110  // Data Rate Command
//#define RX_CTRL_CMD_CODE                 0b10010     // Receiver Control Command
//#define DATA_FILTER_CMD_CODE             0b11000010  // Data Filter Command
//#define FIFO_RST_MODE_CMD_CODE           0b11001010  // FIFO and Reset Mode Command
//#define SYNC_PATT_CMD_CODE               0b11001110  // Synchron pattern Command
//#define RX_FIFO_READ_CMD_CODE            0b10110000  // Receiver FIFO Read Command
//#define AFC_CMD_CODE                     0b11000100  // AFC Command
//#define TX_CONF_CTRL_CMD_CODE            0b1001100   // TX Configuration Control Command
//#define PLL_SET_CMD_CODE                 0b110011000 // PLL Setting Command
//#define TX_REG_WRITE_CMD_CODE            0b10111000  // Transmitter Register Write Command
//#define WKUP_TIMER_CMD_CODE              0b111       // Wake-Up Timer Command
//#define LOW_DUTY_CYCLE_CMD_CODE          0b11001000  // Low Duty-Cycle Command
//#define LOW_BAT_AND_MCU_CLK_DIV_CMD_CODE 0b11000000  // Low Battery Detector and Microcontroller Clock Divider Command
//#define STATUS_READ_CMD_CODE             0b0         // Status Read Command
...
typedef enum {
    FQ_NOT_DEFINE, // 000: Frequence non définit, lecture des microswitch == 0
...
/** Valeur des différents états de l'automate StateMachine_RF_Service */
typedef enum {
    SM_RX_FRAME_IDLE, // en attente de réception
    SM_RX_FRAME_IN_PROCESS, // en cours  de réception
    SM_TX_FRAME_INIT, // en cours d'initialisation
    SM_TX_FRAME_PREAMBLE, // en cours d'envoie du préambule à la trame
    SM_TX_FRAME_SYNCHRON_PATTERN, // en cours d'envoie du motif synchrone pour la détection de trame par le module récepteur
    SM_TX_FRAME_DATA_IN_PROGRESS, // en cours d'envoie des données à transmettre
    SM_TX_FRAME_DUMMY_BYTE, // transmit ending data
    SM_TX_FRAME_CLOSE_TRANSMITTER // transmit ending data
} SM_RF_SERVICE_VAL;

#define NB_PREAMBLE_TO_SEND         (3)     // nombre d'octet de synchronisation à envoyer
#define NB_SYNCH_PATTERN_TO_SEND    (2)     // nombre d'octet de synchronisation à envoyer

#define BUFFER1_USED                (0)     // mémoire tampon 1 utilisé
#define BUFFER2_USED                (1)     // mémoire tampon 2 utilisé

typedef enum {
    NO_ERR, // 00: pas d'erreur en cours
    FFOV, // 01: débordement de la FIFO (mode RX)
    BUFFER_USED_OVERFLOW, // 10: débordement du nombre de buffer utilisé
    BUFFER_FRAME_OVERFLOW // 11: débordement du nombre de caractère à écrire dans le buffer
} RX_ERROR_VAL;

//FIXME: Correction des testes de 2 bits vers 1 bits

/** Status d'état du fonctionnement du module RF */
typedef union {
    uint8_t Val;

    struct {
        unsigned RX_Buffer_In_Use : 1; // flag permettant d'identifier la mémoire tampon en cours d'utilisation (0 pour Buffer1 et 1 pour Buffer2)
...
// Global Variables for external use in main:
extern RF_STATUS_VAL RF_Status; // Status du module RF

extern bool nIRQ_falling_edge; // détection du front descendant de la broche nIRQ / detection of falling edge
...
void FSK_Transceiver_nIRQ_Service(void); // Interrupt on nIRQ RF module
void FSK_Transceiver_Received_Pattern_Init(void); // Initialiser la détection d'une nouvelle donnée
uint16_t FSK_Transceiver_Command(uint16_t cmd_write); // Envoyer une commande au module RF
void FSK_Transceiver_Send_Init(void); // Configuration en mode TX avant l'envoie de donnée
void FSK_Transceiver_Send_Byte(uint8_t data_send); // Transmission d'une donnée par le module RF
unsigned int FSK_Transceiver_ConfigFq(unsigned char freq_selected); // Sélection de la fréquence pour le module RF
void display_STATUS_register_from_RF_module( void );


#endif	/* FSK_TRANSCEIVER */
