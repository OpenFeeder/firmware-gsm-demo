...
void APP_Tasks(void) {
    ...
    //on ne doit pas lire le pit tag
    if (appDataDoor.door_close_bird_sleep) {
        if (appData.state == APP_STATE_RFID_READING_PIT_TAG) {
    ...
    switch (appData.state) {
        case APP_STATE_INITIALIZE:
        {
             ...
             */
            uint8_t i = 0;
            while (i < 20 && !appData.RfModuleInit) {
                if (CMD_3V3_RF_GetValue() == false) {
                    ...
                    i++;
#if defined  (USE_UART1_SERIAL_INTERFACE )
                    printf("\t\tRF Module no power on, we try to power on.\n");
#endif
                }
            }
            if (i >= 20) { // module n
#if defined(USE_UART1_SERIAL_INTERFACE)
                printf("\t\tRF module not power on, system go to sleep!!!!!\n");
#endif  
                ...
                sprintf(appError.message, "RF module not power on, system go to sleep");
        ...
        case APP_STATE_IDLE:
            ...
            if (getDateTime()) {
                ...
                } else if (appData.current_time.tm_sec % 15 != 0 && (print == false)) {
                    print = true;
                }
            }
#endif      
        ...
        case APP_STATE_ERROR:
            if (appData.state != appData.previous_state) {
                ...
                /* If the current error is not related to the USB device, try to log the error */
            ...
            if (appError.number > ERROR_CRITICAL && appError.number < ERROR_TOO_MANY_SOFTWARE_RESET) {
                ...
                if (isDelayMsEnding()) {
                    appData.dsgpr0.bit_value.num_software_reset += 1;
                    DSGPR0 = appData.dsgpr0.reg; // sauvegarde la valeur meme apres le reset
                    DSGPR0 = appData.dsgpr0.reg; // infos, if faut ecrire deux fois d'affiler 
                ...
                /* Send Error With RF module to Master */

                // c'est pour eviter de renvoyer indefiniment l'ereur 
                if (!appError.OfInCriticalError && !appError.errorSend
                        && appError.number != ERROR_RF_MODULE) {
                    appError.OfInCriticalError = true;
                }

                if (!appError.errorSend && appError.OfInCriticalError
                        && appError.number != ERROR_RF_MODULE) {
                    break;
                }
...
void APP_Initialize(void) {
    ...
    //ON OF
    appDataAlarmDefaultOnOF.time.tm_hour = 23;
    appDataAlarmDefaultOnOF.time.tm_min = 50;
