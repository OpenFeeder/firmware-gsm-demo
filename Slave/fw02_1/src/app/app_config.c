...
FILEIO_RESULT read_PIT_tags(void) {
    ...
    if (GO_NO_GO == appData.scenario_number) {
        if (LEFT_RIGHT_LEDS == appDataAttractiveLeds.pattern_number) {
            if (ALL_PIT_TAG_DENIED != appDataPitTag.num_pit_tag_denied_or_color_A && 
                ALL_PIT_TAG_ACCEPTED != appDataPitTag.num_pit_tag_denied_or_color_A) {
...
INI_READ_STATE config_read_ini(void) {
    ...
    if (slave_found) {
        read_parameter = ini_getl("slave", "slave_id", -1, "CONFIG.INI");
        if (-1 == read_parameter) {
            return INI_PB_LOCAL_ID_SLAVE_NO_SET;
        } else {
            appData.slave_id = (uint8_t) read_parameter;
#if defined (USE_UART1_SERIAL_INTERFACE) && defined (DISPLAY_INI_READ_DATA)
            printf("\tlocal slave id... read.\n");
#endif
        }
    }
    /* Clear watch-dog timer because INI read take time */
    ...
    if (logs_found) {
        ...
        read_parameter = ini_getl("logs", "horloge", -1, "CONFIG.INI");
        if (-1 == read_parameter) {
            appDataLog.log_date = true;
            //            return INI_PB_LOGS_RFID;
        } else {
            appDataLog.log_date = (bool) read_parameter;
#if defined (USE_UART1_SERIAL_INTERFACE) && defined (DISPLAY_INI_READ_DATA)
            printf("\tLogs Horloge... read.\n");
#endif
        }
    ...
    //    /* Local ID */
    //    if (local_id_found) {
    //        read_parameter = ini_getl("localId", "id_slave", -1, "CONFIG.INI");
    //        if (-1 == read_parameter) {
    //            return INI_PB_LOCAL_ID_SLAVE_NO_SET;
    //        } else {
    //            radioAlphaTRX_SlaveSetSlaveID((uint8_t) read_parameter);
    //#if defined (USE_UART1_SERIAL_INTERFACE) && defined (DISPLAY_INI_READ_DATA)
    //            printf("\tlocal id slave %d... read.\n", read_parameter);
    //#endif
    //        }
    //    }
...
void getIniPbChar(INI_READ_STATE state, char *buf, uint8_t n) {
    ...
    switch (state) {
        ...
        case INI_PB_LOCAL_ID_SLAVE_NO_SET:
            sprintf(buf, n, "slave id isn't set");
            break;
        default:
